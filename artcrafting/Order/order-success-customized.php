...
$update_sql = "UPDATE `seller` SET `orders`='$new_orders',`order-change`='$new_order_change', `actice-custom-order`= '1' WHERE `name`='$seller_name' AND `id`='$seller_id'";
mysqli_query($conn, $update_sql);

$product_sql = "INSERT INTO `custom`(`id`, `name`, `photo`, `paint-type`, `size`, `seller-name`, `seller-id`, `buyer-name`, `buyer-id`, `product-price`) VALUES ('$order_id', '$product_name','$photo','$type',$product_size,'$seller_name',$seller_id,'$accout_name','$accout_id', '$product_price')";
mysqli_query($conn, $product_sql);


$sql_order = "INSERT INTO `orders`(`order_id`, `payment-id`, `order_date`, `order_status`, `seller_name`, `seller_id`, `product_name`, `product_id`, `buyer_name`, `buyer_id`, `order_type`) VALUES ('$order_id', '$payment_id','$today','Ordered','$seller_name','$seller_id','$product_name','$order_id','$accout_name','$accout_id','Customized')";
